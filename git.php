...
38, 合并分支
	切换到目标分支:
		git checkout master
	然后:
		git merge chu # 意思是把 chu 分支合并到 master 分支

	如果在合并的时候有冲突,则要找到相应的文件,再找到对应的冲突,手动修改;
	然后在按往常一样使用 add , 后 commit
...
49, Git 可以使用四种主要的协议来传输数据: 本地传输, SSH 协议, Git 协议 和 HTTP协议;
	值得注意的是除了HTTP协议之外,其他所有协议都要求在服务器端安装并运行 Git;
...
51, 屏蔽查看指定分支的提交
	git lob 显示的分支名称 -not 屏蔽的分支

52, 查看当前分支和指定分支合并时的完整内容差异
	git diff 指定的分支名称

53, 经常有这样的事件发生,当你正在进行项目中某一部分的工作,里面的东西处理一个比较杂乱的状态,而你想转到
	其它分支上进行一些工作;问题是,你不想提交进行了一半的工作,否则以后你无法回到这个工作点;解决这个问题
	的办法就是 git stash 命令;

	'储藏' 可以获取你工作目录的中间状态-- 也就是你修改的被追踪的文件和暂存的变更--并将它保存到一个未完结
	变更的堆栈中,随时可以重新应用
